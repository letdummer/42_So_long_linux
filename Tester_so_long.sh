...
check_leaks() {
    ...
    local valgrind_log="valgrind_output.txt"
    
    # Executa valgrind com output detalhado
    ...
    # Verifica se há leaks no arquivo de log
    if grep -q "All heap blocks were freed -- no leaks are possible" "$valgrind_log" && \
       ! grep -q "definitely lost:" "$valgrind_log"; then
        rm -f "$valgrind_log"
        return 0 # Sem leaks
    else
        rm -f "$valgrind_log" 
...
# Modify the check_error_map function
check_error_map() {
    local map_file="$1"
    local EXPECTED_OUTPUT="$2"

    # Check if file exists (except for intentionally missing files)
    if [[ "$map_file" != "invent.ber" && ! -f "$map_file" ]]; then
        printf "${RED}$C.[KO] File not found: $map_file${DEF_COLOR}\n"
        ((C++))
        return
    fi
    ...
    if [ $EXIT_CODE -eq 139 ]; then
        printf "${RED}$C.[KO] SEGFAULT${DEF_COLOR}\n"
        ((C++))
        return
    fi

    # Check leaks
    if check_leaks "./so_long" "$map_file"; then
    ...
    if echo "$OUTPUT" | grep -q "^Error"; then
        ...
        ERROR_MSG=$(echo "$OUTPUT" | sed -n '2p')
        if [ -n "$ERROR_MSG" ]; then
            printf "${GRAY}\nExpected output: ${DEF_COLOR}$EXPECTED_OUTPUT\n"
            printf "${GRAY}Your output: ${DEF_COLOR}$ERROR_MSG\n"
        else
...
check_valid_map() {
    ...
    # Use kill -0 instead of ps for process checking on Linux
